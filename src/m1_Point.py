...
Authors: David Mutchler, Vibha Alangar, Dave Fisher, Amanda Stouder,
         their colleagues and Ben Hawkins.
...
# TO (continued)  in file  m0_INSTRUCTIONS.txt, asking questions as needed.
#
# DO (continued): Then implement a class called   Point that has NO METHODS
# (continued)  yet, just the lines that start the definition of any class:
#
...
class Point(object):
    ...
    def move_to(self, x, y):
        self.endx = x
        self.endy = y
        self.distancetraveled = self.distancetraveled + ((self.x-self.endx)**2 + (self.y - self.endy) ** 2) ** (1/2)
        self.x = x
        self.y = y
    ...
    def closer_to(self, point1, point2):
        if self.get_distance_from(point1) < self.get_distance_from(point2):
            return point1
        elif self.get_distance_from(point1) > self.get_distance_from(point2):
            return point2
        else:
            return point1
